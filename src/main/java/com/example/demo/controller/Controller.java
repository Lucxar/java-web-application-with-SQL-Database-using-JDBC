...
public class Controller {
    ...
    public String halloWelt(Model model) {
        ...
        List<KundeDTO> kunden = kundeDao.getAllKunden();
    ...
    public String getFilme(Model model,
        ...
        else if (titel != null && !titel.isEmpty()) {
            // Filtern nur nach Titel
    ...
    public String getVorstellungen(Model model,
        ...
        if (datum != null && uhrzeit != null) {
            vorstellungen = vorstellungDao.getVorstellungenByDatumAndUhrzeit(Date.valueOf(datum), Time.valueOf(uhrzeit));
        } else if (datum != null) {
            vorstellungen = vorstellungDao.getVorstellungenByDatum(Date.valueOf(datum));
        } else if (uhrzeit != null) {
            vorstellungen = vorstellungDao.getVorstellungenByUhrzeit(Time.valueOf(uhrzeit));
        } else {
    ...
    public String getTickets(Model model, @RequestParam(required = false) String email){
        ...
        if (email != null && !email.isEmpty()) {
            List<TicketDTO> tickets = ticketDao.getTicketsByEmail(email);
            model.addAttribute("tickets", tickets);
        }
        else {
            List<TicketDTO> tickets = ticketDao.getAllTickets();
            model.addAttribute("tickets", tickets);
        }
        return "ticket";
    }
    ...
    public String changePassword(@RequestParam("oldPasswort") String oldPasswort,
        ...
        // Ueberpruefe ob passwort und erneute passwort Eingabe uebereinstimmen
